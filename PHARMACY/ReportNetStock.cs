    ...
    public partial class ReportNetStock : Form
    {
        ...
        public void searchNetStockView()
        {
            ...
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT `stock`.`stock_id` AS 'STOCK ID',`supplier`.`name` AS 'SUPPLIER NAME',`drug`.`name` AS 'DRUG NAME',`stock`.`buying_price` AS 'BUYING PRICE',`stock`.`selling_price` AS 'SELLING PRICE',`stock`.`initial_quantity` AS 'ORIGINAL QUANTITY',(`stock`.`initial_quantity`-`stock`.`quantity_sold`) AS 'REMAINING QUANTITY',`stock`.`quantity_sold` AS 'QUANTITY SOLD',(`stock`.`buying_price`*(`stock`.`initial_quantity`-`stock`.`quantity_sold`)) AS 'BUYING AMOUNT',(`stock`.`selling_price`*(`stock`.`initial_quantity`-`stock`.`quantity_sold`)) AS 'SELLING AMOUNT',((`stock`.`selling_price`*(`stock`.`initial_quantity`-`stock`.`quantity_sold`))-(`stock`.`buying_price`*(`stock`.`initial_quantity`-`stock`.`quantity_sold`))) AS 'PROFIT', `stock`.`units` AS 'UNITS', `stock`.`expiry_date` AS 'EXPIRY DATE',`stock`.`batch_no` AS 'BATCH NUMBER',`stock`.`pfno` AS 'REGISTERED BY',`stock`.`registered_date` AS 'DATE REGISTERED',`stock`.`reoder_level` AS 'RE-ODER LEVEL' FROM `drug` JOIN `stock` ON `drug`.`id`=`stock`.`drug_id` JOIN `supplier` ON `supplier`.`id`=`stock`.`supplier_id` WHERE ((`stock`.`status`=1) AND ( `drug`.`name`='" + this.searchDrugTextBox.Text + "') AND  (`stock`.`initial_quantity`>`stock`.`quantity_sold`) AND (DATEDIFF(`stock`.`expiry_date`,'" + dateNow + "')>0) ) ORDER BY `drug`.`name` ASC", con);
